...
export SKIM_DIRONLY_COMMAND="fd --type dir --hidden --follow --exclude .git --color=always"
export SKIM_RIPGREP_COMMAND="rg --hidden --color=always --line-number {}"
export SKIM_DEFAULT_OPTIONS="--ansi"

alias rsk "sk -i -c '$SKIM_RIPGREP_COMMAND' --preview '~/scripts/preview.sh {}'" # Because this is used later it needs to be an alias
...
abbr -a csk "cd \$(sk -c \"$SKIM_DIRONLY_COMMAND\")"
